...
let gateway = "https://byskebox.example.ts.net"

section("main frame: the gateway origin is allowed")
expect("https://byskebox.example.ts.net/", .allow, "gateway root")
expect("https://byskebox.example.ts.net/chat/abc?sid=1#m", .allow, "any path, query, fragment")
expect("https://byskebox.example.ts.net/?token=abc", .allow, "a sign-in link to the gateway itself")
expect("https://BYSKEBOX.example.ts.net/", .allow, "host comparison is case-insensitive")
expect("https://byskebox.example.ts.net:443/", .allow, "explicit default port is the same origin")

section("main frame: anything else leaves the app")
expect("https://github.com/kirodotdev/KiroCrew", .openExternally, "a public site")
expect("https://chonk.example.ts.net/", .openExternally, "another tailnet host")
expect("http://byskebox.example.ts.net/", .openExternally, "same host, http: a different origin")
expect("https://byskebox.example.ts.net:8443/", .openExternally, "same host, other port")
expect("https://byskebox.example.ts.net.evil.example/", .openExternally, "suffix-extended look-alike")
expect("https://evil.example/byskebox.example.ts.net", .openExternally, "gateway name in the path")
expect("https://byskebox.example.ts.net@evil.example/", .openExternally, "gateway name as userinfo")
...
expect("blob:https://byskebox.example.ts.net/6f0c-uuid", .allow, "same-origin blob (a download)")
expect("blob:https://evil.example/6f0c-uuid", .cancel, "foreign blob")
expect("https://byskebox.example.ts.net/", origin: nil, .openExternally,
       "nothing loaded yet: no web origin is trusted")

section("sub-frames are left alone")
expect("https://byskebox.example.ts.net/sandbox-doc/w1", main: false, .allow, "same-origin widget iframe")
