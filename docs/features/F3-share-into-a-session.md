...
| Signed out mid-share, then resumed | session | `/__app-signed-out` before Send (the app's requests alone are refused, so the page cannot bring the sheet up first) → `token-sheet` appears, `share-waiting-count` = 1; `signIn(kind: "cli")` → without any further tap, `share_posts` becomes 1 and `share-result` = `sent:…`; the page's own requests were never refused | dropping the item on 403; no resume after sign-in |
