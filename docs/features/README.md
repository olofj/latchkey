...
| [F19](F19-app-review-access.md) | An Apple reviewer can exercise the app (0.1 rejected for TestFlight beta, 2026-09-25) | **options, not decided** (§4); the reviewer's text is not in hand. 0.1 predates F11, so the reviewer saw a bare `Login` button. Internal testing needs no review at all (§4.0). Encryption declaration consistent; every claimed capability used; iPad claimed but untested; the share extension lacks its own privacy manifest |
