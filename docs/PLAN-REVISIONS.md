...
| O4 | Before M4 / before any M7.5 decision | Check byskebox's dashboard identity settings (e.g. `ssh byskebox 'bash -lc "kirocrew config get dashboard.tailscale"'`) and report `trust_identity`, `allowed_logins`, `pin_scope`. If on, `allowed_logins` must include `owner@example.com`. |
