module latchkey.test/tsnet-harness
...
// The app's own vendored tailscale (R16), so the harness speaks exactly the
// protocol version the app's node does.
replace tailscale.com => ../../app/ThirdParty/libtailscale/tailscale-patched
